 ...
 * 
 * callback) typedef enum {}bool; 을 보자
 */
#include <avr/io.h>
 ...
 * callback) ISR(TCB0_INT_vect)을 보자.
 */
...
int main(void)
{
	 ...
	 * 
	 * callback) #include <avr/interrupt.h>을 보자
	 */
	sei();
...
* 
* callBack) 맨 윗줄에 #define F_CPU 을 보자.
*/
void CLK_Init(void)
{
	CCP = CCP_IOREG_gc;
 ...
 * 
 * callback) #define USER_LED_TOGGLE 을 보자.
 */
void GPIO_Init(void)
{
	PORTF.DIRSET = PIN5_bm;
